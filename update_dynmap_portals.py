...
CONFIG_FILE = "/data/multiverse_worlds.yml"
...
def load_world_name_mapping():
    if not os.path.exists(CONFIG_FILE):
        print(f"[WARN] Fichier de configuration {CONFIG_FILE} non trouvé. Utilisation du mapping par défaut.")
        return {
            "world": "World",
            "world_nether": "Nether",
            "world_the_end": "Ender",
    ...
    try:
        with open(CONFIG_FILE, "r") as f:
            ...
            mapping = data.get("worlds", {})
            if not mapping:
                print(f"[WARN] Aucun mapping trouvé dans {CONFIG_FILE}, utilisation du mapping par défaut.")
                return {
                    "world": "World",
                    "world_nether": "Nether",
                    "world_the_end": "Ender",
        ...
        print(f"[WARN] Impossible de lire {CONFIG_FILE} : {e}. Utilisation du mapping par défaut.")
        return {
            "world": "World",
            "world_nether": "Nether",
            "world_the_end": "Ender",
...
# Charger le mapping
world_name_mapping = load_world_name_mapping()

# ---------------- Vérifications utilitaires ---------------- 
def is_executable_available(cmd):
    return which(cmd) is not None

# ---------------- Test RCON au démarrage ---------------- 
def test_rcon_connection():
    try:
        with MCRcon(RCON_HOST, RCON_PASSWORD, port=RCON_PORT) as mcr:
            resp = mcr.command("list")  # commande simple pour tester
            print(f"[INFO] Connexion RCON OK : {resp}")
            return True
    except Exception as e:
        print(f"[WARN] Connexion RCON impossible : {e}")
        return False
...
def reload_dynmap():
    ...
    # 2️ Essai via Docker exec si docker disponible
...
if __name__ == "__main__":
    # Test RCON dès le lancement
    rcon_ok = test_rcon_connection()
    if not rcon_ok:
        print("[INFO] RCON non disponible, le script utilisera Docker exec si possible pour recharger Dynmap")
    # Lancement initial
    update_dynmap_portals()
